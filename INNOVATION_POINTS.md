...
同一个现象，在 UNet（SD 1.5 / SDXL）、纯 Transformer（HunyuanDiT）、MM-DiT（FLUX）中呈现出完全不同的"指纹"：
- SD 1.5: decoder up_blocks ResNet 单峰
- SDXL: mid_block 单峰
- HunyuanDiT: blocks 11-21 单峰
- FLUX: early single + late single + last joint（双峰+尾端异常）

**FLUX vs HunyuanDiT Pearson r=0.723（同 Transformer backbone）> FLUX vs SD 1.5 r=0.486（不同 backbone）**——backbone attention 结构决定漂移模式，范式影响次要。这些指纹可以用架构拓扑来解释。
...
| SDXL 漂移指纹 | ✅ 19 图，诊断已完成 |
| HunyuanDiT 漂移指纹 | ✅ 19 图，诊断已完成 |
| FLUX 漂移指纹 | ✅ 19 图，完整统计 |
...
| # | 任务 | 当前 |
|---|------|------|
| 1 | SDXL 漂移 + 校正扩展到 19 图 | ✅ 22 图, Δ=+5.23 |
| 2 | HunyuanDiT 漂移 + 校正扩展到 19 图 | ✅ 20 图, Δ=+5.65 |
| 3 | 真实编辑 benchmark（P2P-style prompt-changed editing） | ✅ LPIPS 0.86→0.51 |
| 4 | 指纹与架构拓扑对应关系表 | ✅ 三层预测框架 |
| 5 | Skip connection 因果干预实验 | ✅ Phase 7c (Cut A −27.7%, Noise A +6.4%) |
| 6 | SD 3.5 held-out 预测验证 | ✅ Phase 8a |
| 7 | FLUX 层组消融 + λ scan | ✅ Phase 8b (joint=single=latent, late>early) |
| 8 | SDXL 跨架构因果验证 | ✅ Phase 8c (−11.59 dB) |
| 9 | 跨 prompt 验证 + 编辑验证 | ✅ Phase 8a/8b |
...
|---|------|
| 10 | 论文写作（8 章） |
| 11 | 更多 baseline（LEDITS++, InfEdit, PnP） |
| 12 | 答辩 PPT |
