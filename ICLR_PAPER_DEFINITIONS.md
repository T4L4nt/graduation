...
> 蒸馏 checkpoint（对抗蒸馏训练流程）在 P-multi 下与 base 指纹不可区分；(48)内层峰
> 类 n=2 谱系证据完备：SD1.5 族（4 变体，D_mag ∈ [0.005, 0.023]）与 SDXL 族
> （Turbo，0.0022）——内层峰类谱系内峰位层粒度不变、度量分量弱变异；与斜坡类
> 对照（PixArt-α vs Σ 0.096 可分辨）构成形态类间的系统性差异：C1 分层适用范围
> 声明获得跨类证据支持。
>
> v3.6 补充 (2026-08-14 PixArt-α 斜坡类谱系测试)：(43)PixArt-α P-multi@104 完成
> （104 图 0 失败；pp=0.9643 T27，conc=0.552，sp=0.455，Spearman=+1.000）——斜坡
> 形态谱系稳定（α 与 Σ 同为完美单调斜坡、同末层峰）；(44)斜坡类内形状参数为
> 实例级属性：同谱系对 PixArt-α vs Σ D_mag=0.096 >> 噪声底，但跨谱系对
> FLUX–Σ 0.0011 < 噪声底——形状参数携带实例信息但不追踪谱系（同谱系对差得比
> 跨谱系对大）；(45)C1 适用范围分层声明：峰位层粒度不变性为内层峰类强主张；
> 斜坡类中退化为末层 argmax 稳定（弱主张）；(46)Finding 2 (3) 可分辨性极限
> 修订：斜坡类内实例分辨不保证（同谱系对可分辨至 0.096，跨谱系对可低至噪声底）。
...
| 不对称 | 实测 | 处置 |
|--------|------|------|
| 步数 | SDXL 30 步 vs SD1.5 50 步（summary.json: steps=30） | 显式披露；ΔPSNR 为同图同条件下的组内差，步数对差值方向影响有限，量级跨架构比较需加注 |
| 图集 | 20 生成图（seed=42）vs 19 真实图 | 显式披露；组内差符号稳健，跨架构量级比较加注 |
| cut 位置 | 同名解码器组件（up_blocks.2）而非全局峰（mid_block） | **有意设计**：实验问题就是"同名结构组件是否同角色"。脚本 NOTE 明示。保留，但正文必须写明设计意图 |

−11.59 dB 的方向性结论（SDXL 的 decoder skip 是必要信息通路）是组内干净效应，
不受不对称影响；+2.20 vs −11.59 的跨架构量级对比需按上述披露加注。

### #11 判定（FDR：补算后主张更强）

存盘 results.json 已有 Bonferroni（20/38）。补算 BH-FDR 与 Holm：

| 校正 | cut_a vs original 存活层数 |
|------|---------------------------|
| 未校正 (p<0.05) | 31/38 |
| **BH-FDR** | **31/38**（p 值极小，FDR 不杀任何一层） |
| Holm | 25/38 |
| Bonferroni（已存盘） | 20/38 |

正文报告口径："31/38 层显著（p<0.05），BH-FDR 下 31/38 全部存活，保守
Bonferroni 下 20/38"——多重比较全景，主张不弱反强。报告脚本
（paired_ttest_per_layer）未内置校正的批评成立，但存盘分析已有校正，
且结论对校正选择稳健。

### #16 判定（ρ 数字统一）

存盘权威值：delta_spatial_correlation（cut_a vs original）= **−0.395**
（p=0.014, n=38）；cut_a vs cut_b = −0.395（p=0.014）。CLAUDE.md 的
−0.59 为历史笔误，已标记待统一——**统一为 −0.395**，正文与摘要同步。
...
漂移起源于两条轨迹在跨层级跳跃通路边界的**局部冲突**（§sec:mechanism）。
**位置分离**：切断峰层跳跃通路（Cut A）改善重建（+2.20 dB），切断对照层
（Cut B）无此效应。**内容分离**：以保持通道容量与全局统计、仅摧毁空间
拓扑的高斯替换（Noise A）获得与 Cut A 统计等价的改善（差 0.18 dB；TOST，
Δ=0.5 dB，p=0.0029）——冲突的因果内容是编码器特征的**空间拓扑**。该诊断
预测了一个闭式修正项的位置与方向：它将反演保真度提升至与注意力控制编辑
基线相差 0.2 dB 以内（TOST 等价），且无需逐图优化。

**Noise A 重跑统计**（19 图 × 3 条件 × 50 步，seed 42，与历史值全中复现）：

| 指标 | orig | cut_a | noise_a | paired t (noise vs orig) | 单侧 TOST (noise vs cut) |
|------|------|-------|---------|--------------------------|--------------------------|
| PSNR | 22.46 | 24.66 | 24.84 | t=4.97, p=0.0001, d_z=1.14 | Δ=0.5 dB, p<0.0001 通过 |
| SSIM | 0.634 | 0.693 | 0.697 | t=4.32, p=0.0004, d_z=0.99 | Δ=0.01, p=0.0007 通过 |
| LPIPS | 0.182 | 0.097 | 0.091 | t=−4.80, p=0.0001, d_z=1.10 | Δ=0.02, p<0.0001 通过 |

- Cohen's d 变体标注：方法部分报 **d_z**（配对设计推荐，t/√n）；0.66/0.41/1.39
  为合并标准差变体，二者与 t 值自洽
- 峰层漂移：cut_a −27.7%，noise_a +6.4%（漂移-冲突解耦，事后发现）
- 机械性 L2 膨胀核对（附录 app:noise-a）：噪声功率均值 3.92e5/图，实测漂移²
  增量 6.69e5/图，一阶传播增益 c=1.71（O(1) 量级，与"噪声经单个残差块被动
  传播"一致）；每图相关 −0.118（弱）——核对在均值层面成立，per-image 层面
  如实报告

### §4.2 升级（两段）

**内容分离：拓扑而非容量。** Cut A/B 定位了冲突的位置，但未回答冲突的内容。
Noise A 干预为此设计：将峰层跳跃输入替换为逐张量均值-方差匹配的高斯噪声。
结果：Noise A 显著改善重建（22.46→24.84 dB），且与 Cut A 统计等价——
保留容量与全局统计不足以维持冲突，摧毁拓扑即足以消除它。冲突的因果内容
是编码器特征的空间拓扑。（边界声明：数据未检验"单独摧毁容量"的条件，
"容量毫无作用"不可写。）

**漂移与冲突的解耦（事后发现）。** 两种干预在峰层漂移上走向相反方向：
Cut A 降 27.7%、Noise A 升 6.4%，重建改善却等价。反向分离表明定义 1 的
L2 漂移度量的是轨迹间差异而非冲突：无结构替换输入拉大 L2 距离却不携带
对抗结构。因此漂移剖面的机制性使用限于**定位**：峰位标记冲突进入位置，
漂移幅度不作为冲突强度度量。该分离为事后发现——预注册预测（方向、显著性、
等价性）全部命中，峰层漂移方向不在预注册分支内；附录 app:noise-a 给出
"机械性 L2 膨胀"定量核对。

### Finding 2 补丁

§2.4：D_total=0.018 → D_s=0.018（与定义符号统一）。

### dtype bug 定性（审计附录一行）

NoiseIntervention 潜伏崩溃型：fp32 mean/std 未回 cast 到 fp16（fp16 必崩、
fp32 语义正确）。修复后三数全中复现历史值，历史结果未污染。
