...
LCM="/home/hiaskc/.cache/huggingface/hub/models--latent-consistency--lcm-lora-sdv1-5/snapshots/cf2fced511dbe7e26c8d1d397e728fbab875db4b"
...
def enc_e(pipe):
    ti=pipe.tokenizer("",padding="max_length",max_length=pipe.tokenizer.model_max_length,truncation=True,return_tensors="pt")
...
targets=disc(pipe.unet);hooker=H(pipe.unet);hooker.reg(targets);pe=enc_e(pipe)
print("layers=%d imgs=%d"%(len(targets),len(IMAGES)))
per_img=[]
for imp in IMAGES:
    latent=load_enc(pipe,imp);z_inv=ddim_inv(pipe,latent,pe,50)
    hooker.f.clear()
...
feat_lcm={k:float(np.mean([pi[k] for pi in pi_feats])) for k in pi_feats[0]}
feat_base={"peak_position":0.7632,"concentration":0.5583,"spread":0.5110}
dd=dist_v2(feat_base,feat_lcm)
...
print("LCM: pp=%.4f conc=%.4f sp=%.4f"%(feat_lcm["peak_position"],feat_lcm["concentration"],feat_lcm["spread"]))
print("D_s(base,LCM): D_total=%.6f D_pp=%.6f D_mag=%.6f -> %s"%(dd["D_total"],dd["D_peak_pos"],dd["D_mag"],tag))
json.dump({"LCM":feat_lcm,"baseline":feat_base,"D_s":dd,"verdict":tag},open("/home/hiaskc/Talant/graduation/outputs/p0b_cross_checkpoint/lcm_lora_fingerprint.json","w"),indent=2)
print("Saved.")
