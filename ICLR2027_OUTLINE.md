...
扩散模型反演中的特征漂移长期被视为需要抑制的随机误差。本文发现，漂移并非随机波动，而是由网络架构决定的整体组织结构；我们进一步证明，这种组织结构满足结构指纹的操作性判据——对同一架构在不同 checkpoint、图像集合与扰动条件下保持稳定，对不同架构可辨识。我们在 SD 1.5、SDXL、HunyuanDiT、FLUX 与 SD 3.5 五个架构上系统量化漂移剖面：同一架构内，真实 checkpoint 的权重差异超出随机噪声耐受区三个数量级，而漂移组织保持稳定；跨架构漂移清晰分离，并按 attention 拓扑而非 backbone family 聚类。epsilon-预测与 flow matching 的同架构对照进一步表明，训练目标仅改变漂移的绝对量级而不改变其组织模式——漂移编码的是架构身份，而非训练过程。机制分析揭示，这一稳健性源于功能子空间错位：漂移主要分布于 ResNet 残差流，而微调更新集中于 cross-attention K/V；扰动实验还表明，峰位是跨扰动保持的拓扑不变量。基于这一视角，我们将反演诊断从逐层统计提升为架构级对象：高漂移 skip 连接在不同架构中呈现方向相反的因果作用；一次性的逐架构线性校正即与逐图像优化方法统计等价，同时显著降低计算与存储开销。我们将特征漂移确立为可识别、可利用的结构对象，并提供可复现的逐架构诊断框架。
...
**主张**：Φ(M) 在同一架构内对权重扰动、图像集选择可复现。

**证据 A — 跨 checkpoint 指纹稳定性**：
- SD 1.4 vs SD 1.5 直接指纹比较：D_s = 0.0106 (< 噪声底 p95 = 0.016)
- SD 1.4 ↔ SD 1.5 UNet 权重差异：median ‖ΔW‖/‖W‖ = 0.209（每层相对范数）
- 真实权重差异超出随机噪声耐受区（ε ≤ 1e-4）三个数量级，指纹保持不变

**证据 B — 受控权重扰动剂量曲线**：
- 对 SD 1.5 UNet 施加逐层相对高斯噪声 ε ∈ [1e-6, 1e-3]
- ε ≤ 1e-4 全部 D_total < 噪声底 p95（0.016），D_pp = 0.000 跨越全部有效剂量
- ε = 3e-4：D_total 穿越噪声底但峰位不变
- ε = 1e-3：数值崩溃
- 死亡层级：D_mag 最先退化 → D_shape → D_pp 为硬不变量

**证据 C — Bootstrap 噪声底**：
- 同模型、B=100 次 bootstrap 重采样 19 图：D_total median = 0.0071, p95 = 0.0163
- 稳定区 D_s 与图像重采样噪声不可区分

**证据 D — LOOCV 跨图像稳定性**（Property 1, Phase 1）：
- 19 折留一交叉验证：Pearson r > 0.9999
- Multi-seed: σ/mean = 0.096%

**Figure 3.1**: 权重扰动剂量曲线（D_total/D_pp/D_shape/D_mag vs ε），标注 ΔW=0.209 位置与稳定区边界
