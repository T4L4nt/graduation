...
- 末层删失峰† 标注：斜坡类全部是删失峰（hook 边界截断，峰后回落不可观测）。SD3.5 在 P-t0 下曾是"已验证近末层峰"（block_22 + 47% 回落），P-multi 下移为 block_23 末层（104/104, t=138.4）——免死金牌注明协议范围

**(2) 形态全程稳定（F4, per-σ 分解）**：FLUX 末层集中对每个噪声水平成立（σ ∈ {1.0, 0.663, 0.002} 上 single_37 均为峰）——非 t≈0 端点伪影，是全程一致的架构属性。

**(3) 可分辨性极限（F1, 噪声底）**：斜坡类内部，指纹在三特征空间中**统计不可区分**——FLUX–PixArt D_mag=0.0011 < 同模型 bootstrap 噪声底 median 0.0014（p95 0.0042）。指纹分辨**形态类**，不保证分辨斜坡类内的单个架构。内层峰类内则分辨到层（0.429/0.500/0.684 互异，C1 已证其谱系稳定）。

**(4) 协议轴（P-tT/P-t0/P-multi 双协议对照表）**：指纹依时间聚合方式呈现不同侧面——高噪端单步（P-tT）凸显跨模态边界信号（FLUX s2，全 σ 一致的弱信号 0.13–0.17，Observation 级局部次峰）；全轨迹均值（P-multi）凸显全局形态。FLUX 在 P-tT 下峰于边界、P-multi 下峰于末端——同一架构在两个协议坐标上的两副面孔，非矛盾。

**机制解读（与 §4 互证）**：两类形态对应反演误差的两种组织方式——**局域化冲突**（瓶颈处爆发；§4.2 skip 因果链为其机制）vs **逐深度累积**（无局域化结构，误差随块深度复利式积累）。
...
**适用范围的主动声明（Limitations）**：指纹 = 架构身份，分三层——谱系内识别实例（C1）、跨架构识别形态类（Finding 2）、斜坡类内不保证实例分辨（F1）。
...
- **Band 1 四变体 P-multi**：D_pp 全 0，D_mag ∈ [0.005, 0.023]——C1 承重墙在最严协议下成立
- **法医包 F1–F5**：P-multi 噪声底 bootstrap、防污染核查（原始量级差 37×）、剖面形态检验（Spearman）、per-σ 时间步分解、SD3.5 峰位移复核
- **形态二分定稿**：内层局域型×3（SD1.5/SDXL/H-DiT）vs 末端累积型×3（FLUX/PixArt/SD3.5）
- C3 结晶曲线 (eps+flow) · 随机 init · Skip 因果链 (SD1.5+SDXL) · TOST/BH/FDR · 度量审计 v1→v2 · 层序 canonical 化 · 协议审计（三协议 + 法医）
...
| 项 | 状态 |
|----|------|
| PixArt-α 谱系测试（斜坡类唯一裁决） | 下载被网络阻塞（重试循环在 tmux），P-multi 脚本就绪 |
