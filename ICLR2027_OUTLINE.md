...
扩散模型反演中的特征漂移长期被视为需要抑制的随机误差。本文发现，漂移并非随机波动，而是由网络架构决定的整体组织结构；我们进一步证明，这种组织结构满足结构指纹的操作性判据——正如物理指纹之于身份：对同一架构保持稳定，对不同架构可辨识。

我们在 SD 1.5、SDXL、HunyuanDiT、FLUX、SD 3.5 和 PixArt-Σ 六个架构上系统量化漂移剖面。同一架构内，指纹呈现分量级不变性：拓扑分量（峰位）在七种模型变体中保持恒定——涵盖继续训练（SD 1.4，D_s=0.011）、蒸馏 LoRA（LCM，0.021）、文本编码器置换（0.034）、全量微调（RV，0.043）；度量分量对强微调呈弱变异，而最大变异（0.043）仍不到最近跨架构距离（0.092）的一半。DiT-S/2 的训练结晶曲线揭示，指纹的峰位在训练早期（eps 10k 步/flow 30k 步，相当于总训练量的 2.5%）即写入并锁定，之后对训练步数、训练目标、采样器与条件组件均保持恒定。随机初始化 UNet 有完整的峰结构但峰位完全不同（0.079 vs 训练后 0.763），表明拓扑定义了峰位可以存在的槽位集合，训练选择并锁定其中一个槽位——指纹是拓扑与训练的乘积。

跨架构漂移清晰分离。跨模态交互机制——而非简单的 single/dual-stream attention 二分——决定漂移指纹的聚类结构：同属 single-stream 的 HunyuanDiT 和 PixArt-Σ 反而是最远对（D_s=0.571），而 PixArt 聚类到 MM-DiT 家族（与 SD 3.5 最近，0.149）、LCM/RV/RandText 正确归入 UNet 质心。机制分析揭示，这一稳健性源于功能子空间错位：漂移分布于 ResNet 残差流，而微调更新集中于 cross-attention K/V 投影（Spearman ρ=0.24，其中跨注意力层仅为 0.05）；SD 1.5 的 skip 冲突源在 SDXL 中逆转为必要信息通路（ΔPSNR 分别为 +2.20 和 −11.59 dB）；FLUX 的 joint→single 边界触发双模态同时失稳（image 1.55×, text 3.0×）。基于这一视角，我们将反演诊断从逐层统计提升为架构级对象：最简逐架构线性校正与逐图像优化方法统计等价（TOST 0.2 dB 等价界，P2P vs Ours），同时显著降低计算与存储开销。
