...
| 漂移指纹不由采样范式决定（HunyuanDiT vs FLUX Pearson r=0.723 同 backbone → 相似；FLUX vs SD1.5 r=0.486 不同 backbone → 差异大） | ✅ 独特发现 |
| 跨架构漂移分布统计相似度矩阵 | ✅ 未见类似 |
...
### 5.2 已补齐（2026-07-10 更新）

| 差距 | 状态 | 说明 |
|------|------|------|
| 与 FeatureInject 的明确对比实验 | ✅ | Phase 7 formation_vs_drift 定量对比 |
| User study（可选） | 未做 | 可考虑 |
| 更多定量评估图像 | ✅ | EDICT/NTI 补齐至 19 图；跨 prompt 25 验证 |
| 对标 SOTA 方法的最新版本 | ✅ | 已对比 P2P, EDICT, NTI, ControlNet |
| 论文格式转换（硕士论文 → 8 页 CVPR） | 待做 | 需要大幅压缩 |

### 5.3 可选强化（已完成）

- ~~**新架构验证**：SD3.5~~ **✅ Phase 8a 完成**：预测→证伪→修正→框架细化
- ~~**理论到预测的闭环**：预测新架构漂移指纹~~ **✅ Phase 7c + Phase 8c 完成**：skip 因果干预 + SDXL 跨架构验证
- ~~**FLUX 消融补全**~~ **✅ Phase 8b 完成**：λ scan + 层组消融（joint=single=latent, late_single 最优）
