...
**头牌统计量建议**：计算同架构跨范式的结构距离 d(eps, flow | DiT-S/2)，与已有跨架构距离谱对照。Phase 9 实测：d(eps, flow) ≧ min_cross 在所有归一化方案下均成立。**原假设被证伪**——结构距离无法定量支持"架构≫范式"。证据重心转移到排序稳定性(W=0.938)和定性组织不变性（峰位、加速motif）。
...
2. **"注入位置鲁棒"跨架构复现**：joint_only = single_only = latent_all 三者 per-image PSNR **完全一致**（3 图验证后确认为 19 图 per-image 精确等价，差异 < 1e-12 dB，非均值巧合）。MM-DiT 的 residual stream 使校正信号经 57 层累积后等价——无论在哪一层注入、注入多少层，最终 latent 完全相同。这是比 SD 1.5 random5≈top5 更强的位置不敏感性（SD 1.5 是"近似等价"，MM-DiT 是"精确等价"），将"简单性即优势"从 UNet 推广到 Transformer backbone。

> **验证状态**：精确等价已通过 19 图 per-image PSNR 逐对验证（latent_only = joint_only = single_only，
> 均值均为 14.464631578947，per-image 完全一致）。若为真正的数学恒等（而非数值巧合），
> 机理应为 MM-DiT 的 residual stream 在 linear correction 下满足线性叠加原理——
> 校正信号 f_inv − f_recon 沿 57 层 residual stream 累积后，在 latent 空间的投影不依赖于
> 在哪一层注入。建议在论文中给出严格推导或至少脚注说明这一非平凡性质。
...
| 修复项 | 内容 |
|--------|------|
| Property 1 (Reproducibility) | 补 LOOCV 实验，r=0.999995, σ/mean=0.096% → `outputs/phase1_reproducibility/` |
...
|--------|-------|--------------------------|
| ResNet | **+2.27 ± 0.48 dB** | 6.83 ± 1.34 (n=22) |
| Attention | +1.09 ± 0.48 dB | 6.30 ± 1.11 (n=7) |
| 比率 | 2.1× | 1.08× |

> **统计注意**：MI 差异 t-test p=0.36（Cohen's d=0.43）——ResNet vs Attention 的 MI 差异
> 在统计上不显著。CI 区间存在重叠。论文中"MI 差异仅 1.1×"的表述应附加此限制条件：
> 该 null result 的统计 power 有限（Attention 仅 7 层），意味着不能可靠地断言"无差异"。
> ΔPSNR 的 2.1× 差异不伴随显著的 MI 差异——这暗示两者可能由不同机制驱动，
> 但 MI 的 null result 本身不能作为论证的充分条件。建议在论文中诚实报告 p 值。
...
> **基线行为说明**（§6.1 应补充以下解释，封住"实现可疑"质疑）：
> - **NTI < DDIM (−2.86 dB)**：NTI 优化的是轨迹级 null-text 嵌入，目标函数不是终点像素 PSNR；
>   BLIP caption 质量差的样本上优化可能发散拖低均值。这两条在任何论文里都成立。
> - **EDICT 增益小 (+0.45 dB)**：EDICT 的优势在低步数区间（近似精确可逆），
>   50 步下对 SD 1.5 的边际增益本来就有限。
> - **P2P vs Ours 统计等价**：t-test p=0.0015 但 Cohen's d=0.033（可忽略效应量），
>   Pearson r≈1.000——两者行为完全一致，差异来自数值精度而非算法差异。
> - **ControlNet 不作为反演方法**：Canny 条件生成是不同任务（image→sketch→generation），
>   不参与反演-重建评估。（注：ControlNet 数字仅作为任务差异的参考，不应纳入 inversion 方法对比。）
...
> **数据复用限制**：top-5 漂移层是从同一批 19 张 coco_val 图像诊断确定的（Phase 1）。
> 因此 19-image 的 ΔPSNR 可能包含正向偏差（层选择已见过评估集信息）。
> 100-image 扩展（Δ=+3.30, d=1.34 > 0.79）使用固定层选择在新图像上评估，
> 结果甚至更强，表明偏差即使存在也不大。论文应明确声明这一限制，
> 并将 100-image 结果（而非 19-image 结果）作为主要定量证据。
...
**Top-10 漂移层**（single_2=0.797 + joint_18=0.793 双峰，fp16 实测，从 turnaround t=1 测量）：

漂移呈 **early single 主导 + 尾端 joint 异常** 的分布：

| 组 | n | mean drift | max drift |
|----|---|-----------|----------|
| Joint blocks (image) | 19 | 0.409 | 0.793 (joint_18) |
| **Single blocks (image)** | 38 | **0.550** | 0.797 (single_2) |
| Joint blocks (text) | 19 | 0.165 | 0.496 (joint_18) |

关键发现：
- **Single blocks 漂移 > Joint blocks**（1.34×，fp16 实测）——dual-stream attention 混合 text+image 使特征更稳定
- **Text token 漂移远小于 Image token**（text mean=0.17 vs image mean=0.47，~2.8× 差距，fp16 实测）——文本在反演中高度稳定
- **joint_18 的 text drift 跳升**至 0.50（joint_17=0.25→joint_18=0.50, 2.0x；joint_0..17 均值 0.17→0.50, 3.0x）——跨模态信息瓶颈在最后的 joint block
- 特殊 token 位置分析（bf16 测量，fp16 待核）：EOS/BOS token 漂移最大，语义 token (11-14) > padding token (273-290)
...
**核心发现（2026-07-19 Phase 9 审计修正）**：结构距离的**排序**（非绝对值）高度稳定（Kendall's W=0.938）。attention 拓扑差异可以压倒 backbone family——HunyuanDiT (single-stream) vs FLUX (dual-stream) 是排序最远的配对（rank [9,9,9,9], CV=0.000），尽管都是 Transformer。SD 1.5 vs SDXL 始终最近（[0,0,0,0]）。但 FLUX vs SD 3.5（同 MM-DiT）排远集群（[8,8,6,8]）——**"同 backbone family 接近"不成立**，family 不预测相似性，细粒度 attention 拓扑才预测。**原"SD3.5 held-out 确认"叙事已撤回。** attention 拓扑差异可以压倒 backbone family——HunyuanDiT (single-stream) vs FLUX (dual-stream) 是距离最远的配对（d=1.077），尽管都是 Transformer。漂移指纹不由采样范式决定，而由具体的 attention 结构（single-stream vs dual-stream, joint→single 过渡方向）决定。**特征漂移是架构签名，不是采样器 artifact。**
...
| **FLUX vs SD 3.5** | far-cluster [8,8,6,8] | ~~同 MM-DiT backbone→次近~~ **撤回** — 排远集群，非次近 |
...
| HunyuanDiT vs SD 3.5 | 1.165 | 不同 backbone，实际最远配对 |

**关键发现**：

1. **排序稳定性（2026-07-19 Phase 9）**：Kendall's W=0.938，10对排序跨归一化高度一致。SD 1.5-SDXL 始终最近（[0,0,0,0]），DiT-FLUX 始终最远（[9,9,9,9]）。~~FLUX-SD 3.5 次近~~ **撤回**：fp16重测后排远集群[8,8,6,8]——family不预测相似性，attention拓扑才预测。
...
3. **跨模态交互边界制造漂移 spike**：FLUX joint_18 是 joint→single 的 handoff 点——跨模态上下文在此丢失，text drift 跳升（joint_17=0.25→joint_18=0.50, 3.0x），image drift spike（0.51→0.79, 1.55x）
...
3. DDIM inversion (empty prompt) → 4 条件 reconstruction (target prompt)：
   - baseline（无校正、无 P2P）
   - ours（latent correction, λ=0.7）
   - p2p（cross-attention injection, λ_attn=0.8）
   - ours + p2p（两者叠加）

### 结果（20 对完整评估，SD 1.5，50 步；28 对生成，8 对因编辑冲突未完成）

> **反演协议**：空 prompt 反演（DDIM inversion with empty prompt）。
...
> **LPIPS-PSNR trade-off**：ours 在 12/20 对中 PSNR **低于** baseline（均值 13.05 vs 13.93，−0.88 dB）。
> 校正改善了感知保真度（LPIPS↓）但引入了像素级失真（PSNR↓）。这不是"更好的重建"，
> 而是"不同的重建"——在感知上更像原图但像素值偏差更大。论文应诚实讨论这一 trade-off，
> 而非仅强调 LPIPS 改善。
...
> **解释**：19-image 用空 prompt 反演 → baseline LPIPS 0.856（接近 1，无源信息）；
> 100-image 用 BLIP caption 反演 → baseline LPIPS 0.469（携带源内容信息）。
> 两种协议中 ΔLPIPS 一致（~0.35-0.40），基线绝对值差异是协议差异，非评测栈不一致。
> **论文表中应显式注明反演协议**，避免审稿人将 LPIPS 跳变误读为评测栈不收口。
...
#### CLIP-Dir 双指标验证 ✅ (2026-07-17)

121 对补算 CLIP-Dir（CLIP ViT-L/14），出"内容保持 × 编辑保真"双指标表：

| Condition | LPIPS↓ | PSNR↑ | CLIP-Dir↑ |
|-----------|--------|-------|-----------|
| baseline | 0.469 | 18.25 | **0.115** |
| ours | 0.071 | 25.65 | **0.007** |
| Δ | −0.398 | +7.40 | **−0.109 (p=1.3e−29, d=1.40)** |

> **诚实发现**：校正将 CLIP-Dir 压至接近零——内容保持和编辑保真是同一根杠杆的两端。
> Baseline CLIP-Dir 本身仅 0.115（源 prompt 反演产生的编辑方向本来就很弱），
> 校正进一步将其消除。这不是 bug，是 fundamental trade-off：潜空间校正保护原图内容，
> 天然抵抗编辑方向变化。论文应诚实呈现这一 trade-off，并讨论 λ 的调节作用
> （降低 λ 可在保持和编辑间取平衡）。

#### λ 悬崖曲线 ✅ (2026-07-17)

121 对 original correction λ ∈ [0, 1.0] 全量扫描：

| λ | 0.0 | **0.01** | 0.05 | 0.1 | 0.3 | 0.5 | 0.7 | 1.0 |
|---|-----|------|------|-----|-----|-----|-----|-----|
| LPIPS↓ | 0.469 | **0.329** | 0.099 | 0.074 | 0.071 | 0.071 | 0.071 | 0.071 |
| CLIP-Dir↑ | 0.115 | **0.142** | 0.041 | 0.013 | 0.011 | 0.011 | 0.011 | 0.009 |

> 细粒度实验（22 对，λ∈{0.0,0.01,0.05,0.1,0.2,0.5,1.0}）确认：
> **λ=0.01 处 CLIP-Dir 完整保持（0.142 > 0.140 baseline），但 LPIPS 改善仅 23%**
> （0.429→0.329），未触及校正核心收益。λ=0.05 时 LPIPS 改善跃升至 77%，
> 同时 CLIP-Dir 塌至 0.041（29% 保留）。悬崖在 0.01→0.05 之间，窗口宽度仅 0.04。
>
> **关键发现：λ 前沿是悬崖，不是斜坡。** λ=0.1 就同时触发了 LPIPS 改善饱和
> （从 0.469 降到 0.074，占总改善的 84%）和 CLIP-Dir 塌陷（从 0.115 塌到 0.013，
> 占总损失的 89%）。不存在可以"平衡"的 sweet spot——校正对编辑方向的消解是
> 即时的、全有或全无的。这一发现推翻了此前"降低 λ 可调节"的直觉假设。
>
> **λ=0.01 不是有效 sweet spot**：虽然保留了编辑方向，但仅完成 23% LPIPS 改善
> （0.429→0.329），未触及校正核心收益。λ=0.05 时 LPIPS 改善跃升至 77%，
> 同时 CLIP-Dir 塌至 0.041（29% 保留）。悬崖在 0.01→0.05 之间，窗口宽度仅 0.04。
>
> **前沿刻画（L 形，不是连续权衡）**：过渡窗口（0.01–0.05，宽度 0.04）远窄于
> 平台区间（0.05–1.0，宽度 0.95），比值 ~24 倍。前沿呈 L 形——λ 要么在窗口前
> （保编辑、增益仅 23%），要么跳上平台（满增益、≥77%、编辑被压）。中间地带
> 几乎不存在，即不存在同时达到平台级内容保持与完整编辑保真的 λ。权衡是
> "要么/要么"，而非"多少/多少"。
>
> **平台判据**：平台 = 达到峰值 LPIPS 改善的 ≥90% 区间（0.469→0.071,
> 总改善 0.398，平台进入点 λ=0.05 处 ΔLPIPS=0.370，占总改善的 93%）。
>
> **跨架构注意**：悬崖位置在 SD 1.5 latet 编辑协议上测得，FLUX 等其他架构的
> 瀑布位置可能不同（FLUX 重建任务最优 λ=0.1，与 SD 1.5 的 0.7 不同）。
> 论文 λ 消融图必须标注 0.01 点并用"L 形前沿"语言——客观刻画形状，
> 不写防御性判断。

#### Plan B 负结果 ✅ (2026-07-17)

测试了误差-编辑分离策略（用源 prompt 做一次重建得到"纯净"反演误差
Δ = f_inv - f_recon^src，编辑时注入 f_recon^tgt + λΔ），含 per-timestep 和
endpoint-only 两种变体，在 22 对上验证：

| 变体 | 结果 |
|------|------|
| Plan B per-timestep | λ≥0.3 时 LPIPS 暴增至 0.62+（比 baseline 0.43 更差），校正信号反向破坏重建 |
| Endpoint-only | 所有 λ 下指标与 baseline 完全等同——校正项 ≈ 0，完全不生效 |

> **被证伪的假设**：DDIM 反演的 per-timestep 误差是 trajectory-dependent 的，
> 不同 prompt 下的误差动力学不同，不存在 "prompt-independent 的纯反演误差向量"。
> 潜空间校正通过锚定 inversion trajectory 来保护内容——这个锚定本身就是将
> 重建拉向源图，天然抵抗编辑方向变化。两者在物理机制上不可解耦。

> **叙事影响**：论文中的 "editing plug-in" claim 需要重新框定。校正在
> prompt-changed editing 中的角色不是"编辑增强"（它不增强甚至消除编辑方向），
> 而是"重建锚定"——在编辑流中保护源图结构。这个定位更诚实，也被数据充分支持。
> 与此前 "与 P2P 统计等价" 的 claim 在编辑场景下需物理隔离——P2P 语义是
> "保结构的同时允许编辑发生"，校正在编辑场景做的是反方向的源图锚定。

脚本：`scripts/phase7_editing_100image.py`（主实验）、`scripts/phase7_clip_dir.py`（CLIP-Dir）、
`scripts/phase7_planb.py`（λ 前沿 + Plan B），输出：`outputs/phase7_editing_100image/`
...
| 分集 | 路径 | 数量 | 用途 |
|------|------|------|------|
| coco_val | `data/coco_val/` | 19 张 | 用于层选择(Phase 1) + 定量评估(Phase 2/5) —— 注意层选择与评估共用同一批数据 |
...
|------|-------------------|---------------------|
| 显著变化层 (p<0.05) | **31/38** (Bonferroni: 20/38) | 5/38 |
...
4. **效应是位置特异的（拓扑效应，非容量效应）**：两个 cut 移除等量信息，但 Cut A 改变 31/38 层（Bonferroni 校正后 20/38 层仍显著）而 Cut B 仅改变 5/38 层。Δ 图 r=−0.395（反相关）——不同位置产生不同空间模式，排除容量解释。
...
证据链：FLUX(拟合) → SD3.5(定性拓扑containment, held-out定量确认已撤回) → SD 1.5 skip 干预(因果操纵) → Noise injection(机制分离) → Reconstruction quality(意外发现→更深理解) → SDXL(跨架构负结果→架构特异性) → Editing verification(编辑不退化)
...
## 精度消融（fp16 vs bf16）✅ (2026-07-17)

5 图 × 50 步 SD 1.5，fp16 vs bf16 全流程（反演+重建）latent drift 对比：

| 指标 | 结果 |
|------|------|
| Per-step drift 轨迹 Pearson r | **0.9982**（5 图，min=0.9946） |
| 系统性偏置 | 5.8%（量级差） |
| 偏置 / 跨层 drift 范围（1000×）| **5.8e-05**——可忽略 |

> **结论**：fp16/bf16 量化差异不污染跨架构 drift 距离。
> Per-step 轨迹形状完全相同（r>0.99），量级偏置（5.8%）与跨层信号范围（1000×）不在同一量级。
> 跨架构结构距离结论对测量精度鲁棒。

脚本：`scripts/precision_ablation.py`，输出：`outputs/precision_ablation/`
